 ...
 */
public class PairsFunctionLevelAggregation extends Configured implements Tool {

    /**
     * Emits a word pair and a count of one for each co-occurrence within the
     * specified
     * contextual distance, relying on the Combiner for local aggregation.
     */
    public static class PairsFuncMapper extends Mapper<Object, Text, Text, IntWritable> {
    ...
    /**
     * Acts as both a Combiner and a Reducer to aggregate the local and global
     * counts
     * for each word pair.
     */
    ...
    public int run(String[] args) throws Exception {
        ...
        if (remainingArgs.size() < 2) {
            System.err.println("Usage: PairsFunctionLevelAggregation [-D cooccur.distance=<d>] " +
                    "-topwords <top50words.txt> <input> <o>");
            System.exit(1);
        }

        Job job = Job.getInstance(conf, "Pairs Function-Level Aggregation d=" +
                conf.getInt("cooccur.distance", 1));
        job.setJarByClass(PairsFunctionLevelAggregation.class);

        job.setMapperClass(PairsFuncMapper.class);
        // Combiner = same class as Reducer → map-function-level local aggregation
        job.setCombinerClass(PairsSumReducer.class);
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new PairsFunctionLevelAggregation(), args);
        System.exit(res);
    }
}
